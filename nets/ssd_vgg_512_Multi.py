...
class SSDNet_vgg_512(object):
    def __init__(self):
        # 获取feature map层
        self.anchors_blocks = ['block1','block2','block3','block4', 'block7', 'block8', 'block9', 'block10', 'block11', 'block12']  # block的索引
        # anchor的层
        ...
        self.feat_shapes = [(512,512),(256,256),(128,128),(64, 64), (32, 32), (16, 16), (8, 8),(4,4), (2, 2), (1, 1)]
        self.img_size = 512
        self.num_classes = 2
    ...
    def encode_bboxes(self, ssd_anchors, bboxes, labels):
        ...
        return feat_locs, feat_scores, feat_labels, bboxes
    ...
    def interface(self, inputs, reuse=None, is_training=True,scope='ssd_512_vgg'):
        ...
        with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
            ...
            with tf.variable_scope('fpn_scope','fpn_scope',reuse=reuse):
                '''
                ...
                strides: 卷积核在各个维度移动的步长，a
                4 - D
                list，[stride_batch, stride_height, stride_width, stride_channel]
                padding: 对输入input的填充方法，'SAME'和 'VALID'两种
                '''
                net = slim.conv2d_transpose(end_points['block11'], 256, [3, 3], stride=2)
                net = net + end_points['block10']
                net = slim.conv2d(net, 256, [3, 3])
                end_points['block10'] = net

                net=slim.conv2d_transpose(end_points['block10'],256,[3,3],stride=2)
                net=net+end_points['block9']
                net=slim.conv2d(net,256,[3,3])
                end_points['block9']=net

                net = slim.conv2d_transpose(end_points['block9'], 512, [3, 3], stride=2)
                net = net + end_points['block8']
                net = slim.conv2d(net, 512, [3, 3])
                end_points['block8'] = net

                net = slim.conv2d_transpose(end_points['block8'], 1024, [3, 3], stride=2)
                net = net + end_points['block7']
                net = slim.conv2d(net, 1024, [3, 3])
                end_points['block7'] = net

                net = slim.conv2d_transpose(end_points['block7'], 512, [3, 3], stride=2)
                net = net + end_points['block4']
                net = slim.conv2d(net, 512, [3, 3])
                end_points['block4'] = net

                net = slim.conv2d_transpose(end_points['block4'], 256, [3, 3], stride=2)
                net = net + end_points['block3']
                net = slim.conv2d(net, 256, [3, 3])
                end_points['block3'] = net

                net = slim.conv2d_transpose(end_points['block3'], 128, [3, 3], stride=2)
                net = net + end_points['block2']
                net = slim.conv2d(net, 128, [3, 3])
                end_points['block2'] = net
                net = slim.conv2d_transpose(end_points['block2'], 64, [3, 3], stride=2)
                net = net + end_points['block1']
    ...
    def train_op(self, image, glocalisations, gscores, glabels, bbboxes, sess=None):
        ...
        tf.summary.scalar('trainOP_loss', self.losses)

        global_step = tf.train.get_or_create_global_step()

        optimizer = tf.train.AdamOptimizer(FLAGS.learning_rate).minimize(self.losses, global_step=global_step)

        return optimizer, self.losses, global_step, num, bbboxes

    def train_op_new(self, image, glocalisations, gscores, glabels, bbboxes, sess=None):
        ...
        tf.summary.scalar('trainOP_loss_new', self.losses)
        global_step = tf.train.get_or_create_global_step()
        # 选择待优化的参数
        # output_vars = tf.get_collection(tf.GraphKyes.TRAINABLE_VARIABLES, scope='outpt')
        # optimizer = tf.train.AdamOptimizer(1e-5).minimize(self.losses, var_list=output_vars, global_step=global_step)
        optimizer = tf.train.AdamOptimizer(1e-5).minimize(self.losses,global_step=global_step)
    ...
    def validate_op(self, image, glocalisations, gscores, glabels,ssd_anchors,reuse=None):
        ...
        tf.summary.scalar('validateOP_loss', self.losses)
        ...
        # nms
        boxes_index = self.non_max_supression(fpred_loc, fpred_scores, fpred_labels, 100)
    ...
    def test_op(self, image, ssd_anchors, reuse=True, scope='test'):
        ...
        for i in range(len(ssd_anchors)):
            fpred_loc.append(tf.reshape(pred_loc[i][0], [-1, 4]))
            fpred_scores.append(tf.reshape(tf.reduce_max(pred_cls[i][0], axis=-1), [-1]))
            fpred_cls.append(tf.reshape(pred_cls[i][0], [-1, self.num_classes]))
            fpred_labels.append(tf.reshape(tf.argmax(pred_cls[i][0], axis=-1), [-1]))
            feat_map.append(tf.reduce_sum(tf.argmax(pred_cls[i][0], axis=-1), axis=-1))
            fpred_logits.append(tf.reshape(pred_logit[i][0], [-1, self.num_classes]))
        fpred_loc = tf.concat(fpred_loc, axis=0)
        ...
        fpred_labels = tf.concat(fpred_labels, axis=0)
        fpred_logits = tf.concat(fpred_logits, axis=0)
        fpred_cls = tf.concat(fpred_cls, axis=0)
        pred_mask = tf.greater(fpred_loc, 1)
        pred_fmask = tf.cast(pred_mask, tf.float32)
        fpred_loc = tf.where(fpred_loc < 0, fpred_loc * 0., fpred_loc)
        fpred_loc = tf.where(pred_mask, pred_fmask * 1., fpred_loc)
    ...
    def non_max_supression(self, pred_loc, pred_scores, pred_labels, max_output_size=100):
