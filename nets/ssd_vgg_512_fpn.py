...
from basenets.vgg import vgg_16,vgg_arg_scope

class SSDNet(object):
    def __init__(self):
        self.anchors_blocks = ['block6', 'block7', 'block8', 'block9', 'block10']  # block的索引
        self.anchors_steps = (32, 16, 8, 4, 2)
        self.offset = 0.5
        self.anchors_ratios = [[2, .5],
                               ...
                               [2, .5],
                               [2, .5]]
        self.anchors_sizes = [(60,60),
                              (120, 120),
                              (180, 180),
                              (250,250),
                              (300,300)]
        self.feat_shapes = [(16, 16), (32, 32), (64, 64), (128, 128), (256, 256)]
        self.img_size = 512
        self.num_classes = 5
    ...
    def encode_one_bboxes(self, anchors, bboxes, labels, threshold=0.5):
        ...
        def body(i, feat_ymin, feat_xmin, feat_ymax, feat_xmax, feat_score, feat_labels):
            ...
            mask = tf.logical_and(mask,tf.greater((bbox[2]-bbox[0])*(bbox[3]-bbox[1]),0))
    ...
    def interface(self, inputs, reuse=None, is_training=True,scope='ssd_512_vgg'):
        ...
        with slim.arg_scope(vgg_arg_scope()):
            ...
            with tf.variable_scope('fpn_layers','fpn_layers',[net],reuse=reuse):
                net=slim.conv2d_transpose(net,512,[3,3],stride=2,scope='trans_conv1')+end_points['block5']
                end_points['block7']=net

                net=slim.conv2d_transpose(net,512,[3,3],stride=2,scope='trans_conv2')+end_points['block4']
                ...
                net = slim.conv2d_transpose(net, 128, [3, 3], stride=2, scope='trans_conv4') \
                      + end_points['block2']
